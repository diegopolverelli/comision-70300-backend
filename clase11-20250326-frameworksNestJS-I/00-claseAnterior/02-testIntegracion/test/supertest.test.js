...
describe("Pruebas sobre router /api/pets", function(){
    ...
    describe("Pruebas básicas (sin subir imágenes)",()=>{
        ...
        it("Si envío datos correctos de una mascota, se da de alta en DB", async()=>{
            ...
            let {statusCode, body}=await requester.post("/api/pets")
                                        .send(mockPet)
                                        .set("authorization", "BEARER PRUEBA...")
                                        .set("Cookie", "cookie01=100; cookie02=200")
    ...
    describe("Pruebas complejas (con envío de imágenes en la request)",async()=>{
        it("Si envío datos de una mascota, con una imagen a la ruta /api/pets/withImage con POST, graba la mascota en DB", async()=>{
            let mockPet={name:"Roger",specie:"testing20250319", birthDate: new Date().toUTCString()}

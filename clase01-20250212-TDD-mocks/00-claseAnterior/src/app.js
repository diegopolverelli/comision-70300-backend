...
import {fork, exec, execFile} from "child_process"
const PORT = 3000;
...
app.get('/calcu', (req, res) => {
    ...
    execFile("calc.exe", (error)=>{
        if(error){
            ...
            return res.status(400).json({error:`Error al ejecutar calculadora`})
        }
...
app.get('/dir', (req, res) => {
    ...
    exec("dir /s", (error, resultado)=>{
        if(error){
            res.setHeader('Content-Type','application/json');
            return res.status(400).json({error:`Error al ejecutar dir /s`})
        }
